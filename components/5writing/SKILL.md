...
撰写前先读取 `../../references/paper-structure-first-prize-writing.md`，并建立三项结构合同：
...
正文采用“问题目的与物理图像 → 建模理由 → 数学表达 → 求解方法 → 结果解释 → 验证”的叙事顺序。公式前先说明为什么需要该关系，公式后解释变量、约束或几何意义；图表出现前交代读图目的，图表之后指出支持了什么结论。使用自然过渡连接条件和推论，避免机械重复“首先、其次、最后”，也避免连续用“如图”“由图”起句。
...
每个问题段必须包含实际使用的方法或模型、关键机制或约束、一个最能回答题问的主要结果（定量问题含单位与必要场景）、一项关键验证或敏感性结论，以及结果含义。其余全精度残差、搜索容差和中间指标放在正文或附录，不在摘要中堆叠。定性问题报告经验证的类别结论及证据类型，不得为满足格式捏造数值。若不同合理口径导致不同答案，使用“在……口径下……；若……则……”明确分开，不得把不同场景的数值写成同一结论。摘要不得使用项目符号、表格、复杂公式、引文、内部文件名或“效果较好”等空泛表述。

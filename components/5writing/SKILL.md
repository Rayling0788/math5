...
校准稿必须先判断问题结构，再建立内部映射。顺序推导或单层估计按 `模型建立/逐步推导 -> 模型求解 -> 求解结果 -> 结果分析与检验` 展开；确需联立的优化、方程组或耦合决策按 `代表性推导 -> 变量/约束/目标解释 -> 模型汇总 -> 模型求解 -> 求解结果 -> 结果分析与检验` 展开。改写已有论文时，不得把原有多个有效小节合并成一个“更精炼”的小节；复杂问题的角色颗粒度只能保留或细化。先完成结构改写，再做句式和连接词校准。若校准稿仍把公式、搜索、主结果和多项残差压入同一段，即使语句通顺也判为校准失败。

“模型汇总”应学习范文的组织逻辑而非照抄其标题。写作前先判断：若各式依次计算中间量、一个估计模型直接输出参数或概率、或自然递推已经清楚表达计算顺序，则沿真实动作链写作，不另设汇总也不写“本问无独立目标函数”占位句。只有当多类未知量、约束和目标/求解准则必须联立，或需要集中交代 MATLAB 的完整求解器输入时，才设置模型汇总。此时应先用肉眼可辨认的段落或题意化小标题分别解释变量、约束和目标或真实求解准则，再由汇总块联立；汇总不得首次引入符号、约束或目标。若上游模型只向下游输出一个概率、系数、状态或边界，说明该接口并只汇总需要联立的下游层，不复制两套公式。块后说明已知输入、待求量、索引范围和 MATLAB 求解顺序。内容超过一页或必须缩小字号时，拆块或移出次要恒等式，不得退化为公式墙。
...
中文论文初稿、重写稿和摘要定稿均读取 `../../references/evidence-preserving-natural-prose.md`。先锁定 claim、数值、单位、公式含义、场景、引用和结论强度，再检查成簇出现的空泛拔高、装饰性分析、无名权威、抽象名词串、强行对称、空洞预告、聊天残留和泛化结尾。单个“因此”“进一步”“本文”或被动句不构成问题，只有重复模式遮蔽对象、动作或因果时才改写。

按“结构与证据—具体动作—保真对照”完成三遍：第一遍恢复段落角色和公式前后接口；第二遍用真实对象、计算动作、条件和结果替换仪式化措辞；第三遍逐项核对是否新增、删减、强化或弱化任何事实、数字、单位、场景、引用、限制或推荐。最终论文保持中性学术语域，不注入口语、个性化评论或为避免术语重复而进行同义词轮换。
...
7. 对含递推、分段路径、碰撞判定或连续优化的复杂问题，至少显式区分模型建立/代表性推导、一般模型或必要汇总、模型求解、求解结果、结果分析与检验五类角色。标题按题意和计算结构改写：顺序递推可以用一般递推替代模型汇总，联立优化才设置汇总；不得把这些角色压进一个小节或一个长段。先以一个相邻单元、一个路径段、一个候选碰撞对或一个局部事件完成代表性推导，再推广到全链或全域。
8. 全文原则上只配置一张总体技术路线图，用于表达跨问题依赖。局部算法流程图按必要性配置：保留条件是存在循环、分支、回退或停止判定，并且与已有公式、几何图或数据图不重复。局部图放在对应算法小节，正文在图前说明读图目的、图后解释分支如何产生输出。

正文采用“问题目的与物理图像 → 建模理由 → 代表性对象推导 → 一般递推/分类或必要汇总 → 求解方法 → 结果解释 → 验证”的叙事顺序。机制复杂时，将建模、求解、结果和分析分开；是否单列模型汇总由耦合程度决定，不由题号或模板决定。不得把公式、算法、结果和验证压缩在同一段。公式前点明实际依据，公式后说明未知量、物理意义或下一步求解；图表前交代读图目的，图表后指出支持的结论。

必要的“模型汇总”只能出现在代表性推导之后，用来帮助读者回看耦合关系，不能代替第一次解释。汇总前后必须能顺次回答六个问题：变量是什么且单位为何；哪些约束限定变量；哪个函数负责描述、拟合或选择；这些关系如何联立；已知量和未知量分别是什么；MATLAB 按什么顺序求解。若关系本来按单向顺序逐步产生输出，则用自然推导和一般公式回答这些问题，不强行加汇总。若正文首次出现全局目标或成组约束时，读者尚未见过具体对象的推导，应先补局部说明，再保留必要汇总。
...
提交稿直接描述最终模型，不叙述“本文如何修改上一版”，也不保留“下面将介绍”“希望有所帮助”“如有需要可继续”等对话或编辑痕迹。不要为了显得完整而固定列出三点、为每问复制同一种句式，或用“重要意义、广泛前景、奠定基础”代替具体结果和适用范围。
...
摘要定稿后再做一次反模板化复核：各问不得只是把题号和模型名替换进同一句式；删除背景拔高、算法名堆叠、空泛“结果合理”和泛化前景，只保留能改变答案或解释的动作、数值、条件与限制。复核不得改变任何 claim 或舍入。

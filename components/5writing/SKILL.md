...
“模型汇总”只能出现在代表性推导之后，用来帮助读者回看完整计算关系，不能代替第一次解释。若正文首次出现统一递推式、全局目标或成组约束时，读者尚未见过一个具体局部对象的推导，应先补局部说明，再保留汇总块。
...
摘要首段通常用 1--2 句说明研究对象、总体任务和贯穿各问的共同建模主线，不展开背景历史，不提前堆放结果。共享路径、数据口径或假设可在首段统一说明；各问只重复会改变本问数值或结论的场景条件。每个问题段通常用 2--4 句完成：第一句点明本问要求和采用的数学结构；中间句用“先……再……进而……”概括真实执行顺序，并仅在目标函数或关键约束确实决定答案时带出它们；末句给出题目要求的直接答案组，再从验证、趋势、解释或适用限制中选择最关键的一项收束。模型名称必须与其实际作用连写，例如说明它把哪种运动、碰撞或优化关系转化成什么可计算量，不得只列术语，也不得只写“建立模型并用 MATLAB 求解”。

...
## 图片证据规划
- 不按问题或章节规定固定张数，不为凑数量绘图。
- 每个计算问题覆盖机理/输入、主要结果、验证/敏感性三类证据；可由一张协调多视图图承担多个角色。
- 全文默认只放一张总体技术路线图；局部流程图仅用于真实循环、分支、回退、多初值或停止判定。
- 局部公式优先正视或剖视构型，趋势与比较使用真实数据图，第三维具有真实含义时才使用 3D 图。

---

# 数据特征输出规范（关键！）

**每张图的绘图代码后，必须输出关键数据特征，并实际查看导出的 PDF/PNG。**
结构化打印用于保证写作数值一致，不能替代视觉检查。每张核心图都要在原始尺寸和论文最终尺寸下检查视角、字体、坐标轴、单位、图例、色条、箭头遮字、标签裁切和重叠。
...
## 题意多解的路线/结构优化问题特别注意
- 先登记比较对象、成本或长度包含的组成、固定端点、可移动进入/退出位置、参数自由度和可行区域。
- 题面与快速推导明显支持一种解释时，完整求解主解释并记录排除理由，不机械复制多套模型。
- 两种同等合理解释会翻转“能否优化”等核心答案时，正文显式披露：主口径完整求解，另一口径可用独立数值分支或充分的解析证明、不变量、反例处理。
- 下游问题必须继承同一解释和场景键，不得把一条路径的几何与另一条路径的速度、容量或推荐值拼接。

...
5. **可视化方案**：推荐的图表类型及用途
6. **读者推导合同**：先选一个具体对象或局部构型，写明已知量、待求量、第一条关系的依据、该式先求出的量、推广方式，以及最终如何变成代码使用的统一模型

读者推导合同与求解器合同必须分开。几何、机构、碰撞、递推和分段路径问题，先用局部构型、辅助线、投影、守恒或相切关系说明模型怎样产生，再给向量式、统一下标和约束集合。不得只给模型名称、总公式和算法清单；若无法从题面与模型说明一个具体局部情形，应明确指出证据缺口。

...
- 非数据图常见选择：总体技术路线图、必要的局部算法流程图、数据处理流程图、变量关系图、模型结构图、指标体系图。默认只用一张总体路线图；不得按子问题数量机械配流程图。
- 图中文字、坐标轴、图例和 caption 应与论文语言一致。图内不要写长标题，标题交给论文 caption。
- 同类图表风格要统一，颜色要能灰度区分，避免过度装饰、阴影和无意义渐变。

## 非数据图工具选择

- DrawIO 适合总体技术路线图，以及确有迭代、判断、回退、多初值筛选或停止条件的局部算法流程图，也适合数据处理 Pipeline、指标体系层次图、模型选择决策树、甘特图、小规模网络拓扑和简单概念框架。简单递推和数值结论不使用 DrawIO。
- MATLAB 适合由模型参数决定的 2D/3D 几何、局部坐标、尺寸链、接触关系、边界条件、结构剖面和物理场；这类图必须与求解器使用同一参数源。
- TikZ 适合不依赖求解参数的精确数学标注、公式节点、复杂连线、变量关系或因果路径；用户指定 MATLAB 时不得用 TikZ 替代参数化模型示意图。
...
- 速查：需要可编辑逻辑流程时优先 DrawIO；需要真实参数几何、尺寸、局部基向量或物理场时优先 MATLAB；只在不依赖模型参数且未限定运行工具时考虑 TikZ；复杂真实场景才考虑生成式图片。
...
- 中文摘要先用一段简短总述，再按每个顶层子问题各写一个独立自然段；建议显式使用“针对问题一……”等衔接，不得将多问压成一个混杂段落。
- 每个子问题段必须覆盖：建立了什么模型或数学结构、执行了什么求解算法或数值方法、哪些目标或关键约束决定答案、主要结果是什么（定量问题含关键数值、单位和必要场景）、如何验证以及结果意味着什么。
...
2. **问题重述与问题分析**：不是照抄题面；在一个“问题分析”大节内按实际问数设置独立小节，每问说明输入输出、任务本质、依赖、难点、备选方法、选择理由和预期证据
3. **模型假设**：全篇仅保留一个一级“模型假设”大节；共同与各问特有假设在节内分组，每条说明依据、适用范围、影响与失效处理，并在正文中引用
4. **符号说明**：所有后文用到的符号在此定义，有量纲标注
5. **模型建立与求解**：每个子问题独立成节；先说明建模思路和代表性推导。顺序推导或单层估计按真实计算链继续，联立方程或优化问题才在变量、约束和目标/准则分别解释后设置“模型汇总”，最后说明求解方法
...
- 摘要把所有问题挤在一个段落 → 逐问分段并分别写清方法、结果、验证和含义。
- 问题分析只有总体思路 → 按每个顶层问题拆成独立小节，补齐输入输出、难点、备选与取舍。
- 模型假设散落在各问或重复成多个一级章节 → 合并到唯一的“模型假设”大节，并在后文引用假设编号。
- 每问机械套用“变量—约束—目标—汇总” → 先判断是否真正联立；顺序推导保留自然动作链，只有耦合求解才汇总，且不得虚构目标或重复上游模型。
- 为体现创新机械组合 AHP、CRITIC、PSO、BP、LSTM 等模型 → 只保留与题目证据、数据规模和验证能力相匹配的模型，并记录可信备选与拒绝理由。
...
下表仅用于识别可能的候选方法，不构成选型捷径。使用前必须验证数据量、适用条件、物理或业务机制、可解释性和基线表现；不得为了“高级”或“创新”机械叠加模型。

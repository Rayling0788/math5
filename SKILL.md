...
For a mechanism-heavy, estimation, prediction, or optimization question, end the derivation with a visible `模型汇总` block before the numerical-solution subsection. Build it in the reader's dependency order: first define indexed variables, roles, domains, units, and data sources; then state the physical, statistical, logical, boundary, feasibility, and anti-leakage constraints that restrict them; only then define the response/transition relation and the objective, likelihood, or loss used to estimate or choose them. Finally combine those pieces into the exact system consumed by MATLAB. Keep only core relations and group them by `变量域 -> 约束 -> 函数/目标 -> 联立模型`; after the block state known inputs, unknown outputs, index ranges, and MATLAB execution order. If estimation and downstream decision are separate layers, show two linked blocks and name the exported quantity between them. Do not replace a representative derivation with an unexplained equation wall or repeat intermediate identities.

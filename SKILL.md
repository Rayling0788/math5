...
For a mechanism-heavy, estimation, prediction, or optimization question, end the derivation with four visibly separate labeled blocks before the numerical-solution subsection: `变量定义`, `约束条件`, `目标函数/求解准则`, and `模型汇总`. First define indexed variables, roles, domains, units, and data sources without mixing in feasibility equations. Next state the physical, statistical, logical, boundary, feasibility, and anti-leakage constraints, and identify which variables each constraint restricts. Only after those two blocks define the objective, likelihood, loss, risk, fitting criterion, or equation-solving criterion. A non-optimization problem must explicitly state that it has no independent objective function and name the actual solution criterion; never invent an objective to fill the template. Finally combine only previously defined variables, constraints, and criteria into the exact system consumed by MATLAB. A compact table may support a block but cannot merge these roles into the same row or equation wall. After the summary state known inputs, unknown outputs, index ranges, and MATLAB execution order. If estimation and downstream decision are separate layers, show two linked summaries and name the exported quantity between them. Do not let the final summary become the first place where a variable, constraint, or objective is explained.
...
- Do not accept a mechanism-heavy, estimation, prediction, or optimization chapter that derives several equation families but lacks a compact model-summary block before numerical solution. The block must expose variables and units, constraints, the response/objective/loss function, the combined solver-ready system, known inputs, unknown outputs, index ranges, and MATLAB execution order; a formula collection with no dependency order is not a model summary.

...
4. Read domain references only when their trigger applies: field solver, multi-objective metric, attachment, MATLAB model view, paper structure, CUMCM official format, or final QA.
...
Audit a reference paper before learning from it. Separate transferable presentation strengths such as direct-answer abstracts, scenario tables, algorithm narration, diagnostics, and visual pacing from evidence claims that require independent verification. Inspect the declared evaluation unit, split logic, preprocessing boundary, out-of-fold stacking, label taxonomy, metric definition, and appendix code when available. Never imitate a high accuracy, zero-error claim, global-optimum claim, deep-model choice, or clinical guarantee merely because it appears in an awarded paper. Use the presentation pattern only after replacing unsupported evidence with the current project's accepted results and uncertainty.

For a rewrite-only request, freeze accepted values, units, scenarios, assumptions, and model meaning, not the original exposition. WriterAgent may rebuild subsection order, replace a solver-first formula wall with a concrete local derivation, add equivalent symbolic steps, and reconnect accepted figures to the reasoning without rerunning the numerical model. If the reports, equations, and code do not support a trustworthy reader-facing derivation, return an exposition gap to the modeling stage; do not preserve an opaque structure merely because the user prohibited recalculation, and do not invent a bridge that changes the model.

Calibrate the Chinese abstract as a separate genre rather than shortened body text. Follow `references/chinese-paper-language-style.md`: each question paragraph must make the task, executed action chain, direct answer group, and answer-changing scenario clear; include an objective, constraint, validation, or limitation only when it helps determine or qualify that answer. Replace unexplained proof labels such as “an invariant implies” with the shortest concrete relation that names what is fixed and what follows. Do not turn the abstract into a registry-like field checklist.
...
Create one overall modeling workflow diagram after the dependency graph stabilizes and place it near the start of `问题分析`. For the editable DrawIO route, add a local algorithm flow only when a real loop, branch, fallback, or multi-stage decision materially benefits from a diagram. For the MathModel Workshop image2-compatible route, do not apply that subjective filter: follow the recovered product logic exactly by preparing one overall route, one route for every structured subproblem, and a model-structure/mechanism schematic only for tasks whose `stage == proposed`; create extra mechanism/model-structure/scene schematics only when the user explicitly requests them. Build route content from the structured plan plus real solver code. Build a `proposed` schematic from the topic, proposed-task record, global method/baseline/innovation claims, real code, real result summary, and existing method chapter, with code taking precedence. If image2 cannot be invoked directly, save and give the complete prompt to the user, require the original generated PNG back, and keep the figure `WAITING_FOR_USER_IMAGE` until it passes the thesis 8.0/10 review in `components/4drawio/references/image2-figure-workflow.md`. Never use image2 for data-bearing results, validation plots, response surfaces, fields, or precise parameterized geometry; generate those from real data with MATLAB. Retain the editable source or prompt sidecar plus PDF/PNG exports, and verify arrow direction, Chinese text, formulas, spacing, and final-size readability.
...
- Do not accept a coupled chapter whose solver input cannot be reconstructed because several variable, constraint, and objective or solution-criterion families were never brought together after explanation. Require a compact summary only when those families must be solved or optimized jointly. Conversely, reject a summary forced onto a sequential derivation, single-layer estimate, or one-way interface when it merely repeats the preceding chain, invents an objective, or introduces terminology that has no later role. Fixed heading names are never the acceptance criterion.
...
- For a CUMCM submission governed by the 2026 revision, do not accept a table of contents, an electronic paper whose first page is not the one-page Chinese abstract page numbered 1, margins below 2.5 cm, main text over 30 pages, identity leakage, a missing support-file appendix, excerpt-only source code, a paper or support archive over 20 MB, or inconsistency between the paper and support package. Follow `references/cumcm-2026-format-contract.md` over bundled-template defaults.
- Do not accept a rewrite that changes only wording while preserving a solver-first or formula-wall structure. Accepted numbers may remain frozen, but every mechanism-heavy question still needs a reader-facing local derivation before its generalized computational model; unsupported explanatory gaps must return upstream instead of being hidden by polished prose.
- Do not accept zero, duplicate, or scattered top-level model-assumptions sections. All shared and question-specific assumptions must live inside one coherent `模型假设` section and state basis, scope, effect, and failure treatment. For a competition paper, keep this section to at most five key assumptions: merge overlapping sampling, independence, measurement, boundary, or label premises by theme, and move implementation details to variable definitions, constraints, or limitations. Do not hide a genuinely answer-changing premise merely to meet the count.
...
- Do not accept record-level validation when repeated persons, devices, sites, dates, batches, or other stable groups can leak across folds. All preprocessing, feature selection, resampling, tuning, stacking, calibration, and threshold selection must remain inside the training scope; stacked features must be out-of-fold.
- Do not headline Accuracy for an imbalanced task without prevalence, baseline, PR-AUC or another recall-sensitive metric, probability scoring/calibration, and a declared independent evaluation unit.
- Do not accept inconsistent label contracts, undefined performance terms, or semantic substitution between attainment proportion, confidence, assay accuracy, reliability, and prediction accuracy.
- Do not accept deep, stacked, or highly tuned models without stable leakage-free improvement over a credible simple baseline. Training curves, regularization, and early stopping are not generalization evidence.
- Do not call a stochastic search globally optimal without a certificate or exact comparison. Require all-run distributions, feasibility rates, seeds, and a small-instance exact baseline when available.
- Do not accept zero-miss, near-perfect, clinical-guarantee, or similar strong claims from a reused or sparse sample without independent cluster-respecting uncertainty evidence.
...
- Non-data diagrams and image2 handoff: `components/4drawio/SKILL.md`

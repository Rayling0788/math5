...
Keep upstream files under `components/` unchanged. Apply Codex-specific role and quality rules from the files listed under Resource Map.
...
7. Read each stage Skill immediately before starting that stage.
...
11. Read `components/5writing/SKILL.md`; for Typst also read `components/typst-author/SKILL.md`. Dispatch one `WriterAgent` with exclusive ownership of `paper/`.
12. Read `components/6verity/SKILL.md`; dispatch `VerifierAgent` for final QA. Fix and recheck every blocking finding.
